...
func jumpTween():
	...
	#tween.tween_property(self, "scale", jumpStretchSize, .1)
	#tween.tween_property(self, "scale", Vector3(3,3,3), .1)
	pass

# Get Player Input
func get_input(_delta):
	var move_direction := Vector3.ZERO
	move_direction.x = Input.get_axis("move_forward", "move_back")
	move_direction.z = Input.get_axis("move_right", "move_left")
